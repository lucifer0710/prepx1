...
function parseAlternateWeekInfo(roomStr, subjectStr) {
    let room = (roomStr || '').trim();
    let subject = (subjectStr || '').trim();
    let alternateText = null;

    const combined = (room + ' ' + subject).toUpperCase();

    if (combined.includes("FIRST") || combined.includes("1ST") || combined.includes("WEEK 1") || combined.includes("WEEK1")) {
        if (combined.includes("ALTERNATE")) {
            alternateText = "Alternate : Week 1";
        }
    }
    if (!alternateText && (combined.includes("SECOND") || combined.includes("2ND") || combined.includes("WEEK 2") || combined.includes("WEEK2"))) {
        if (combined.includes("ALTERNATE")) {
    ...
    room = room
        .replace(/FIRST\s*WEEK\s*ONWARDS\s*ALTERNATE\s*WEEK/gi, '')
        .replace(/FIRSTWEEKONWARDSALTERNATEWEEK/gi, '')
        .replace(/ALTERNATE\s*FROM\s*FIRST\s*WEEK/gi, '')
        .replace(/ALTERNATEFROMFIRSTWEEK/gi, '')
        .replace(/ALTERNATE\s*FROM\s*SECOND\s*WEEK/gi, '')
        .replace(/ALTERNATEFROMSECONDWEEK/gi, '')
        .replace(/\(Alternate\s*:\s*Week\s*[12]\)/gi, '')
        .replace(/Alternate\s*:\s*Week\s*[12]/gi, '')
        .replace(/\s+/g, ' ')
        .trim();
...
function buildSlotInnerHTML(displaySubject, rawRoom, type, code, explicitAltText) {
    ...
    const tagClass = (type || 'Lecture').toLowerCase();
    ...
    return `
        ...
        <div class="tags">
            ...
            ${alternateText ? `<span class="tag alternate">${alternateText}</span>` : ''}
        </div>
    `;
}
...
function pickElective(opt) {
    ...
    const htmlContent = buildSlotInnerHTML(opt.subject, opt.room, opt.type, opt.code);
...
function renderBatchData(data) {
    ...
    data.classes.forEach(classObj => {
        ...
        if (hasExplicitOptions || isSlashMultiElective) {
            ...
            const htmlContent = buildSlotInnerHTML(displaySubject, rawRoom, type, code);
